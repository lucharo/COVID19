...
theme: united
#resource_files: 
# - ProcessedData/* # some files from this folder are not being found by rmarkdown::find_external_resources
---
...
```{r LoadData}
plts = list.files('Plots/')
plts = plts[stringr::str_detect(plts, '.rds')]
plts = unlist(lapply(str_split(plts, '\\.'), '[[', 1))
for (file in plts) {
  eval(parse(text = paste0(file, ' = readRDS("Plots/',file,'.rds")')),envir = .GlobalEnv)
}
